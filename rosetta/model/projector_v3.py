...
v3 variants replace the original KV concat input path with hidden-state inputs.
"""
...
@register_model
@capture_init_args
class C2CHiddenConcatProjectorV3(Projector):
    """
    V3 projector with both sharer and receiver hidden states:

        source hidden + receiver hidden -> concat -> Linear down projection -> MLP

    After the input projection, this keeps the same C2C MLP, dynamic weighting,
    gate, and residual KV update path.
    """
    ...
    def __init__(
        ...
        source_hidden_dim = source_hidden_dim if source_hidden_dim is not None else source_dim * source_num_heads
        target_hidden_dim = target_hidden_dim if target_hidden_dim is not None else target_dim * target_num_heads
        ...
        input_dim = source_hidden_dim + target_hidden_dim
        out_dim = target_dim * target_num_heads
    ...
    @staticmethod
    def _slice_to_length(hidden_state: Tensor, seq_len: int, name: str) -> Tensor:
        if hidden_state.dim() != 3:
            raise ValueError(f"{name} must have shape (B, N, hidden_size); got {tuple(hidden_state.shape)}")
        if hidden_state.size(1) != seq_len:
            if hidden_state.size(1) < seq_len:
                raise ValueError(f"{name} sequence length is shorter than target KV length: {hidden_state.size(1)} < {seq_len}")
    ...
    def forward(
        ...
        if not isinstance(source_hidden_pair, (tuple, list)) or len(source_hidden_pair) != 2:
            raise ValueError("C2CHiddenConcatProjectorV3 expects (source_hidden, target_hidden)")

        source_hidden_state, target_hidden_state = source_hidden_pair
        target_key, target_value = target_kv
        batch_size, target_heads, seq_len, target_head_dim = target_key.shape

        source_hidden_state = self._slice_to_length(source_hidden_state, seq_len, "source_hidden_state")
        target_hidden_state = self._slice_to_length(target_hidden_state, seq_len, "target_hidden_state")

        if source_hidden_state.size(0) != batch_size or target_hidden_state.size(0) != batch_size:
            raise ValueError("source/target hidden batch sizes must match target KV batch size")
        ...
        _nvtx_push("fuser_v3.hidden_concat")
        ...
        key_hidden = self.key_in(hidden_cat)
        value_hidden = self.value_in(hidden_cat.to(dtype=target_value.dtype))
        _nvtx_pop()
...
register_model("HiddenConcatC2CProjectorV3")(C2CHiddenConcatProjectorV3)
