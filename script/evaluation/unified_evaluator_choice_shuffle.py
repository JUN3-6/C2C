...
class UnifiedEvaluator:
    ...
    @staticmethod
    def _answer_index_to_letter(answer_num: Optional[int]) -> Optional[str]:
        return chr(65 + int(answer_num)) if answer_num is not None else None

    def _parse_mmlu_redux_answer_index(self, example: Dict[str, Any]) -> Optional[int]:
        """Return the current MMLU-Redux answer index after any choice remapping."""
        ...
        if error_type == 'wrong_groundtruth' and example.get('correct_answer') is not None:
            ...
            if answer and answer[0] >= '0' and answer[0] <= '9':
                return int(answer)
            return ord(answer.upper()[0]) - ord('A')
    ...
    def _maybe_shuffle_mmlu_redux_choices(
        ...
        """Deterministically shuffle MMLU-Redux choices and remap the answer.

        This is used to test whether a model follows answer content or a fixed
        A/B/C/D prior. The returned example has the same question and a new
        choice order, with answer/correct_answer remapped to the new letter.
        """
        shuffle_cfg = self.eval_config.get("choice_shuffle", {})
        if not isinstance(shuffle_cfg, dict) or not bool(shuffle_cfg.get("enabled", False)):
            return example
        ...
        seed = int(shuffle_cfg.get("seed", 0))
        key = "||".join([
            str(seed),
            str(subject),
            str(question_id),
        ...
        rng_seed = int(hashlib.sha256(key.encode("utf-8")).hexdigest(), 16)
        rng = random.Random(rng_seed)
        perm = list(range(len(choices)))
        rng.shuffle(perm)

        shuffled = dict(example)
        new_answer_num = perm.index(int(old_answer_num))
        shuffled["choices"] = [choices[idx] for idx in perm]
        shuffled["answer"] = int(new_answer_num)
        if example.get("correct_answer") is not None:
            shuffled["correct_answer"] = str(new_answer_num)
        shuffled["_choice_shuffle_enabled"] = True
        shuffled["_choice_shuffle_seed"] = seed
        shuffled["_choice_shuffle_perm"] = ",".join(chr(65 + idx) for idx in perm)
        shuffled["_choice_shuffle_original_true_answer"] = self._answer_index_to_letter(old_answer_num)
        shuffled["_choice_shuffle_shuffled_true_answer"] = self._answer_index_to_letter(new_answer_num)
        return shuffled
    ...
    def parse_answer(self, example: Dict[str, Any]) -> Optional[str]:
        ...
        if self.dataset_name == "mmmlu":
            ...
            return self._answer_index_to_letter(self._parse_mmlu_redux_answer_index(example))
        elif self.dataset_name == "longbench":
    ...
    def evaluate_subject(self, subject: str, model, tokenizer, device: torch.device, 
        ...
        if self.dataset_name == "math-500":
            dataset = load_dataset(self.dataset_config["dataset_name"], **load_kwargs)
        elif self.dataset_name == "gsm8k":
            base_subset = "main" if is_virtual_split else subject
            dataset = load_dataset(self.dataset_config["dataset_name"], base_subset, **load_kwargs)
        elif self.dataset_name == "openbookqa":
            dataset = load_dataset(self.dataset_config["dataset_name"], **load_kwargs)
        elif self.dataset_name == "gpqa":
            ...
            dataset = load_dataset(self.dataset_config["dataset_name"], subject, **load_kwargs)
        else:
            dataset = load_dataset(self.dataset_config["dataset_name"], subject, **load_kwargs)
        ...
        for i in tqdm(sample_indices, desc=f"Evaluating {subject} ({self.eval_config['answer_method']})"):
            try:
                original_example = test_data[i]
                example = self._maybe_shuffle_mmlu_redux_choices(original_example, subject, i)
            ...
            # 对于LongBench，立即保存结果
                ...
                cot_log_entry = {
                    ...
                    'question_id': i,
                    'true_answer': true_answer if self.dataset_name != "longbench" else None,
                    'choice_shuffle_enabled': bool(example.get('_choice_shuffle_enabled', False)),
                    'choice_shuffle_seed': example.get('_choice_shuffle_seed', ''),
                    'choice_shuffle_perm': example.get('_choice_shuffle_perm', ''),
                    'original_true_answer': example.get('_choice_shuffle_original_true_answer', ''),
                    'shuffled_true_answer': example.get('_choice_shuffle_shuffled_true_answer', ''),
                    'pred': pred,
                    'is_correct': is_correct,
        ...
        if all_cot_logs:
            if self.dataset_name != "longbench":
                ...
                fieldnames = [
                    ...
                    'true_answer', 'choice_shuffle_enabled', 'choice_shuffle_seed', 'choice_shuffle_perm',
                    'original_true_answer', 'shuffled_true_answer',
                    'pred', 'is_correct', 'answer_method',
...
def main():
    ...
    # Keep CUDA_VISIBLE_DEVICES so callers can remap physical GPUs while the
    # recipe uses logical GPU IDs such as [0, 1].
